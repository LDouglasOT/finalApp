...
const LikedProfile = ({route,navigation}) => {
    ...
    const convo = {
      ...
      try{
        ...
        if(response.status == 201 || response.status == 200){
          navigation.navigate("Chatting",{"conversationId":response.data[0],"credentials":creds})
        }
        }else{
          navigation.navigate("Chatting",{"conversationId":conversation[0],"credentials":creds})
        }
  ...
  const Like = async () => {
    try {
      ...
      const response = await axios.post("http://192.168.18.5:3001/api/like", data,{headers:headers})
