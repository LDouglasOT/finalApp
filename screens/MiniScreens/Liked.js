...
const Liked = ({ like, tag }) => {
  ...
  const getLikedData = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://192.168.100.57:3001/api/mylikes", json, { headers: headers });
      if (res.status == 200) {
    ...
    return (
      <View style={styles.grid}>
        ...
        <View style={{ width: '100%', height: '100%', display: 'flex', align: 'center', justifyContent: 'center', paddingHorizontal: 20 }}>
          ...
          />
          <Text style={{ color: COLORS.pink, fontSize: 18, marginLeft: 'auto', marginRight: 'auto', flexWrap: 'wrap' }}>Getting Liked Profiles...</Text>
        </View>
      </View>
    )
  }
