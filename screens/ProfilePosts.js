...
const PersonalPosts = ({ navigation, userid, FirstName, LastName }) => {
    ...
    async function getMoments() {
        try {
            ...
            const res = await axios.get(`http://192.168.18.14:3001/api/personalmoments/${userid}`, { headers: headers })
            if (res.status == 200) {
    ...
    const promotedUsers = async (gender) => {
        ...
        try {
            ...
            const res = await axios.post("http://192.168.18.14:3001/api/promoted", data, { headers: headers })
            if (res.status == 200) {
