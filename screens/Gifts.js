...
const Gifts = ({navigation}) => {
    ...
    useEffect(()=>{
      const getGifts = async()=>{
        ...
        const response = await axios.get("http://192.168.18.5:3001/api/getgifts",{headers:headers})
  ...
  const fetchMyGifts = async() =>{
    ...
    const response = await axios.get(`http://192.168.18.5:3001/api/getusergifts/${data.id}`,{headers:headers})
    if(response.status == 200){
      setMyGiftsLoading(true)
    ...
    const handlePayment = async() => {
      ...
      try{
        ...
        const response = await axios.post("http://192.168.18.5:3001/api/buyingift",payload,{headers:headers})
