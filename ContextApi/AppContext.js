...
export function LoginProvider({ children }) {
  ...
  const checksubscription = async () => {
    ...
    const response = await axios.post("http://192.168.100.57:3001/api/subscribe", data, { headers })
  ...
  const getCurrentUser = async () => {
    ...
    const response = await axios.get(`http://192.168.100.57:3001/api/currentuser/${user.id}`, { headers })
    if (response.status == 200) {
